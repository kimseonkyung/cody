				...
				<c:if test="${paging.currentPageNo gt 3}">  											  <!-- 현재 페이지가 3보다 크다면(즉, 4페이지 이상이라면) -->
		            ...
		            <c:choose>
		                <c:when test="${i eq paging.currentPageNo}"> 
		                      <li class="active"><a href="javascript:gopage(${i}, ${paging.maxPost})"><span class="fontNum">${i}</span></a></li> <!-- 1페이지부터 5개씩 뽑아내고, 1,2,3페이지순으로 나타내라-->
		                </c:when>
		                	<c:otherwise>
		                    <li><a href="javascript:gopage(${i}, ${paging.maxPost})"><span class="fontNum">${i}</span></a></li> 
							</c:otherwise>
					</c:choose>
					</c:forEach>
