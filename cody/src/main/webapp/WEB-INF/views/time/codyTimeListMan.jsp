   ...
   <div
	...
	<div class="container">
		<nav class="nav nav-underline">
			...
			<a class="nav-link col-4 mr-auto" href="${pageContext.request.contextPath}/codyTimeListMan" style="font-size: 22px; font-weight: 600; background-color: #17a2b8; color: #fff;">Men</a>
      ...
      <div class="col-md-10 order-md-2">
         <div class="my-3 p-3 bg-white rounded box-shadow">
            <ol class="card-item-ol">
               ...
               <c:forEach var="irrM" items="${irrM}">

                  <li class="card card-item-li" style="margin-bottom: 50px;display: inline-block;"><a
                     href="${pageContext.request.contextPath}/codyListView?cody_idx=${irrM.codyidx}">
                        <img class="card-img-top card-item"
                        src="${pageContext.request.contextPath}/uploadfile/codyphoto/${irrM.codyimage}"
                        alt="Card image cap">
                  </a>
                     <div class="card-body"
                        style="overflow: hidden; position: relative; padding-top: 10px; padding-left: 10px; padding-right: 10px; padding-bottom: 10px;">

                        <c:forEach var="item" items="${irrM.itemtime}" begin="0" end="2"
                           step="1">
                        ...
                        <div class="card-main" style="float: left; height: 40px;margin-top: 3px;">
                           <c:choose>
			<c:when test="${not empty irrM.memberphoto}" >
		<img class="rounded-circle"
                              src="${pageContext.request.contextPath }/uploadfile/memberphoto/${irrM.memberphoto }"
		...
		<img class="rounded-circle" src="${pageContext.request.contextPath }/uploadfile/member.png" style="width: 40px; height: 40px;"> 
                           ...
                           <div style="float: right; padding-left: 15px;">
                              <div>
                              <c:set var="memberId" value="${irrM.memberid}"/>
                              ...
                              <div>
                                 <p
                                    style="vertical-align: inherit; margin-bottom: 0px; height: 20px; font-size: 5px">${irrM.codyheight}cm
                                    ${irrM.codyage}세</p>
                              </div>
                           </div>
                        </div>
                        <c:choose>
                           <c:when test="${loginInfo.member_idx eq irrM.memberidx}">
                              <a href="${pageContext.request.contextPath }/codyUpdate?cody_idx=${irrM.codyidx}" class="btn btn-info" style="float:right; width: 73px; height: 42px;">수정</a>
                           </c:when>
                           <c:otherwise>
                              <button class="btn btn-info" style="float: right;width: 73px; height: 42px;padding-top: 0px;padding-bottom: 0px;" onclick="idx(${irrM.memberidx})">
