...
public class PageDto {
	...
	int maxPost; // 페이지당 표시될 게시물 최대 개수 및 현재 게시물 개수
	...
	public void makePaging() {
		...
		if (maxPost == 0) // 페이지당 표시되는 최대 게시물 수가 0개라면
			setMaxPost(5); // 최대 표시되는 게시물 수를 5개로 설정한다. (기본 값 설정)
		...
		boolean isNowFirst = currentPageNo == 1 ? true : false; // 위의 조건에따라 현재 페이지가 1이면 true값을 아니면 false값을 isNowFirst에
																// 넣는다.
		boolean isNowFinal = currentPageNo == finalPage ? true : false; // 현재페이지와 마지막 페이지가 동일(ex. 9페이지)라면 true값을, 아니면
																		// false값을 isNowFinal에 넣는다.
		...
		if (!isNowFirst)
			setPrevPageNo(((startPage - 1) < 1 ? 1 : (startPage - 1))); // startPage(초기값 1)이 1보다 작으면 1페이지를 표시하고, 그게아니라면
																		// 시작페이지 -1을 표시
																		// 예를 들어 8페이지에 있는경우 이전을 누르면 초기페이지(6)에서 -1을 한
																		// 5페이지가 표시된다.
		...
		// 7페이지가 있는 블록의 끝(endPage)페이지는 10페이지이며, 그 10페이지에 1을 더한 값이 finalPage(17페이지)보다 크면
		// finalPage를, 아니면 끝(endPage)페이지의 +1을한 11페이지를 표시.
